...
Original file is located at
    https://colab.research.google.com/drive/1cgnmlaHauiODbc6grUZh9xZeHL_lcuWy
...
def daily_return(df, price_col='AAPL'):
    """
    Calculate daily returns for a DataFrame with stock prices.
    Assumes df has a column with prices named price_col (default: 'AAPL').

    Returns a new DataFrame with an added 'daily_return' column.
    """
    # Make a copy to avoid modifying original DataFrame
    df = df.copy()

    # Calculate daily return using .pct_change()
    # This is the safest and simplest way
    df['daily_return'] = df[price_col].pct_change() * 100 # Multiply by 100 to get percentage

    return df

 usage:

stocks_daily_return = daily_return(stocks_df, price_col='AAPL')
