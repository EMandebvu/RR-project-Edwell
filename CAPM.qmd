...
This section loads the necessary R packages—**`plotly`** for interactive plots, **`tidyverse`**, **`ggplot2`** for data manipulation and visualization and **`readr`** for data reading.
...
A quick look at the first few rows is performed to verify the structure and contents of the dataset.(Ri is AAPL, Rm is market return(s&p 500)
...
An interactive line chart is generated using Plotly, visualizing the cumulative returns of selected stocks and the market over time. Each line represents a different asset, indexed to a starting value of 100.
...
The mean daily return is calculated for each stock and the market, summarizing the average performance over the observed period.
...
A scatter plot is created to visualize the relationship between daily returns of a single stock Ri (AAPL) and the overall market returns (S&P 500).

CAPM assumes a linear relationship between an individual stock’s returns and market returns. This plot visually inspects that relationship, helping to assess if the stock’s returns move consistently with the market as CAPM suggests.

```{r}
ggplot(df, aes(x = Rm, y = Ri)) +
  geom_point(color = "darkgreen") +
  labs(
    title = "Ri (AAPL) vs Rm(Market Returns)",
    x = "Rm",
    y = "Ri"
  ) +
  theme_minimal()
...
Beta is a fundamental parameter in CAPM, indicating the stock’s systematic risk relative to the market. Alpha helps identify any abnormal returns beyond what CAPM predicts. This step formally quantifies the core assumptions of CAPM by fitting the expected linear relation.
...
Beta values highlight how sensitive each stock is to movements in the overall market. Stocks like *TSLA* and *MGM* show high beta, indicating they tend to move more sharply in response to market changes—making them more volatile and risk-prone. In contrast, stocks such as *T* and *IBM* have lower beta values, suggesting more stable price movements with less responsiveness to market fluctuations.

**Alpha (Excess Return vs Market):**\
Alpha measures the portion of a stock’s return that exceeds what would be expected based on its market exposure. Positive alpha values, seen in *AMZN*, *GOOG*, *TSLA*, and *AAPL*, suggest these stocks consistently outperformed the market beyond what CAPM predicts. Negative alphas in stocks like *T*, *MGM*, and *IBM* indicate underperformance, where returns fell short of their risk-adjusted expectations.
...
The validation statistics offer insight into how well the CAPM explains each stock's return in relation to the market. Across the board, **p-values are effectively zero**, confirming that the relationship between market returns and individual stock returns is statistically significant.
